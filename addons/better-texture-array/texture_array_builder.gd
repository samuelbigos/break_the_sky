...
func _get_image_from_channels(channels, size: Vector2, format: int = Image.FORMAT_RGBAF):
...
func _load_image(path: String, size: Vector2, format: int = Image.FORMAT_RGBAF) -> Image:
	var img
	if path.begins_with("#"):
		img = Image.new()
