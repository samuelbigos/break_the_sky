...
public class BoidAlly : BoidBase
{
	...
	[Export] private PackedScene MicoBulletScene;

	public Sprite _sprite;
	public AudioStreamPlayer _sfxShot;
	public AudioStreamPlayer _sfxHit;
	public AudioStreamPlayer _sfxHitMicro;
	...
	public float _microBulletTargetSearchTimer;
	private BoidBase _microBulletTarget = null;
	public float _microBulletCD;
	
	public override void _Ready()
	{  	
		_sprite = GetNode("Sprite") as Sprite;
		_sfxShot = GetNode("SFXShot") as AudioStreamPlayer;
		_sfxHit = GetNode("SFXHit") as AudioStreamPlayer;
		_sfxHitMicro = GetNode("SFXHitMicro") as AudioStreamPlayer;
		_damagedParticles = GetNode("Damaged") as Particles2D;
	
		(GetNode("Trail") as Trail).boid = this;
		_sprite.Modulate = ColourManager.Instance.Secondary;
		_baseScale = _sprite.Scale;
		
	}
	
	public void Init(Node2D target, Game game)
	{  
		_target = target;
		_game = game;
	}
	...
	{  
		...
		if(!_destroyed)
		{
			...
			{
				Vector2 linearComp = _velocity.Normalized() * steering.Length() * steering.Normalized().Dot(_velocity.Normalized());
	...
	//		_sprite.modulate = Colours.Secondary;
		...
		if(_shootCooldown > 0.0f)
		{
			...
			_sprite.Scale = from.Slerp(_baseScale, 1.0f - t);
